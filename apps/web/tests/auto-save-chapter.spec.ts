...
test.describe('Auto-Save Chapter Questions', () => {
  ...
  test('auto-saves when clicking Add Question on new chapter with content', async ({
    page,
  }) => {
  ...
  test('does NOT auto-save when clicking Add Question without changes on existing chapter', async ({
    page,
  }) => {
    // Setup: Mock lecture data
    const lecture = {
    ...
    // Click Add Question WITHOUT making changes - should NOT trigger auto-save
    await page.getByRole('button', { name: 'Add Question' }).click();
